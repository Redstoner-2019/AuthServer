...
public class Token {
    ...
    public Token(String username, long createStamp, long maxAge, String password) {
        ...
        try {
            this.token = hashPassword(uuid.toString() + uuid2.toString() + uuid3.toString() + uuid4.toString());
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    public static Token createToken(String username){
        return new Token(username,System.currentTimeMillis(), TimeUnit.DAYS.toMillis(3),"");
    }
